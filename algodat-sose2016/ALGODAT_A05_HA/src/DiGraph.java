...
public class DiGraph implements Graph {
	...
	public List<Node> breadthFirstSearch(Node startNode){
		...
		while (!allSearched(nodeList))
		{
			...
			{
				Node next = nodeList.getFirst();
				next.status = Node.GRAY;
				List<Node> whiteSuccs = next.getWhiteSuccessors();
				for (Node n : whiteSuccs)
					n.status = Node.GRAY;
				nodeList.addAll(whiteSuccs);
			}
		}
	...
	private boolean allSearched(List<Node> nodeList)
	{
		for (Node n : nodeList)
		{
			if (n.getWhiteSuccessors().size() != 0)
				return false;
		}
		return true;
	}
	...
	public List<Node> depthFirstSearch(Node startNode){
		...
		while (!allSearched(nodeList))
		{
			for (pos = 0; pos < nodeList.size(); pos++)
			{
				if (nodeList.get(pos).status == Node.WHITE)
					break;
			}
			
			Node firstWhite = nodeList.get(pos);
			firstWhite.status = Node.GRAY;
			List<Node> whiteSuccs = firstWhite.getWhiteSuccessors();
			for (Node n : whiteSuccs)
			{
				nodeList.add(pos, n);
			}
